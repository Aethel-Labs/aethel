...
function getApiConfiguration(apiKey: string | null, model: string | null, apiUrl: string | null) {
  ...
  const finalModel = model || (usingCustomApi ? 'openai/gpt-4o-mini' : 'moonshotai/kimi-k2.5');
...
function buildSystemPrompt(
  ...
  const currentModel = model || (usingDefaultKey ? 'moonshotai/kimi-k2.5 (default)' : 'custom model');
