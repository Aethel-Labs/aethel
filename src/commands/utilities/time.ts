...
const POPULAR_CITIES = [
  'London', 'New York', 'Tokyo', 'Paris', 'Sydney', 'Los Angeles', 'Chicago', 'Toronto', 'Berlin', 'Madrid',
  'Mexico City', 'Moscow', 'Beijing', 'Seoul', 'Mumbai', 'São Paulo', 'Johannesburg', 'Cairo', 'Istanbul', 'Dubai'
];
...
export default {
  ...
  async execute(client: BotClient, interaction: ChatInputCommandInteraction) {
    ...
    if (shown.length === 1) {
      ...
      const fieldDay = await client.getLocaleText('commands.time.embed.field_day_of_week', locale);
      ...
      let title = await client.getLocaleText('commands.time.embed.title_single', locale, { city: match.city, country: match.iso2 });
      title = `${title} ${flag}`;
      const author = {
      ...
      const footer = {
        text: await client.getLocaleText('commands.time.embed.footer', locale, { timezone: match.timezone }),
      };
      ...
      const description = await client.getLocaleText('commands.time.embed.multi_city_description', locale, { city });
      const fields = await Promise.all(shown.map(async match => {
        const now = moment().tz(match.timezone);
        const day = now.format('dddd');
        const time = now.format('h:mm A z');
        const date = now.format('MMMM D, YYYY');
      ...
      const title = await client.getLocaleText('commands.time.embed.title_multi', locale, { city });
      const author = {
        name: `🕒 ${await client.getLocaleText('commands.time.embed.author', locale)}`,
        iconURL: undefined,
      };
      const footer = {
        text: shown.length > 1
          ...
          : await client.getLocaleText('commands.time.embed.footer', locale, { timezone: shown[0].timezone }),
      };
      const embed = new EmbedBuilder()
        .setTitle(title)
        .setAuthor(author)
  ...
  async autocomplete(client, interaction: AutocompleteInteraction) {
    ...
    const filtered = POPULAR_CITIES.filter(city => city.toLowerCase().includes(focused.toLowerCase())).slice(0, 25);
