...
async function getTodosForUser(userId: string) {
  try {
    const { rows } = await pool.query('SELECT item FROM todos WHERE user_id = $1 AND done = FALSE ORDER BY created_at ASC', [userId]);
...
async function getDoneForUser(userId: string) {
  try {
    const { rows } = await pool.query('SELECT item FROM todos WHERE user_id = $1 AND done = TRUE ORDER BY completed_at ASC', [userId]);
    return rows.map(r => r.item);
  } catch (error) {
...
export default {
  ...
  async execute(client: BotClient, interaction: ChatInputCommandInteraction) {
    ...
    if (sub === 'add') {
      ...
      try {
        await pool.query('INSERT INTO todos (user_id, item, done) VALUES ($1, $2, FALSE)', [userId, item]);
      ...
      const item = interaction.options.getString('item', true);
      try {
        const { rowCount } = await pool.query('UPDATE todos SET done = TRUE, completed_at = NOW() WHERE user_id = $1 AND item = $2 AND done = FALSE', [userId, item]);
        if (rowCount === 0) {
          await interaction.reply({
      ...
      if (todos.length === 0 && done.length === 0) {
        ...
        const listTitle = (await client.getLocaleText('commands.todo.list_title', locale)) || '📝 To-do List';
        const todoTitle = (await client.getLocaleText('commands.todo.todo_title', locale)) || 'To-Do';
        const doneTitle = (await client.getLocaleText('commands.todo.done_title', locale)) || 'Done';
  ...
  async autocomplete(client: BotClient, interaction: AutocompleteInteraction) {
    ...
    const todos = await getTodosForUser(userId);
    const filtered = todos.filter(t => t.toLowerCase().includes(focused.toLowerCase())).slice(0, 25);
